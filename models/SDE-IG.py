...
class SDE-IG(nn.Module):
    ...
    def forward(self, batch, return_data="pred",casual_mix=False,c_pred=False,num_newedge=2,num_label = 3,adapt_select = True, alpha=0.2):
        ...
        for i in range(numlabel_batch):
            ...
            for casual_part,causal_attr,casual_weight in zip(casual_edge_indices[y],causal_edge_attr_dict[y],causal_weight_dict[y]):
                ...
                spu_part_set = torch.unique(spu_part).detach().cpu().tolist()
                casual_part_set = torch.unique(casual_part).detach().cpu().tolist()
                
                # 修复 1：安全采样。如果虚假子图节点数不足，允许重复采样(choices)或用 0 占位
                if len(spu_part_set) >= num_newedge:
                    new_spu = random.sample(spu_part_set, num_newedge)
                elif len(spu_part_set) > 0:
                    new_spu = random.choices(spu_part_set, k=num_newedge)
                else:
                    new_spu = [0] * num_newedge  # 如果切割后子图为空，使用 0 占位

                # 修复 2：因果子图同理
                if len(casual_part_set) >= num_newedge:
                    new_cau = random.sample(casual_part_set, num_newedge)
                elif len(casual_part_set) > 0:
                    new_cau = random.choices(casual_part_set, k=num_newedge)
                else:
                    new_cau = [0] * num_newedge
                    
                new_edges = torch.tensor([new_cau, new_spu]).to(device)
                
                # 修复 3：安全获取新边属性
                if len(batch.edge_attr) >= num_newedge:
                    new_attr = batch.edge_attr[:num_newedge].to(device)
                else:
                    new_attr = torch.zeros((num_newedge, batch.edge_attr.size(1) if batch.edge_attr.dim()>1 else 1)).view(num_newedge, -1).squeeze().to(device)
    ...
    def get_embedding(self, batch):
        """专门用于提取图的全局特征表示 (用于t-SNE可视化)"""
        self.eval() # 确保在 eval 模式
        
        # 1. 节点特征经过 GNN 编码
        h = self.gnn_encoder(batch)
        
        # 2. 根据您的 c_in 配置，重新组装 Batch 对象以送入分类器
        if self.c_in.lower() == "raw":
            full_batch = batch
        else:
            full_batch = DataBatch.Batch(
                batch=batch.batch,
                edge_index=batch.edge_index,
                x=h,
                edge_attr=batch.edge_attr,
        ...
        # 3. 通过分类器，强制 get_rep=True 以截取中间层的 Embedding
        batch_pred, batch_rep = self.classifier(full_batch, get_rep=True)
        
        # 返回 32 维的图级表示 (Graph Representation)
        return batch_rep
    
    @torch.no_grad()
    def get_causal_embedding(self, batch, adapt_select=True):
        """专门提取不变子图(Causal Graph)的嵌入表示"""
        self.eval()
        h = self.gnn_encoder(batch)
        device = h.device
        row, col = batch.edge_index
        ...
        pred_edge_weight = self.edge_att(edge_rep).view(-1)
        # 推理阶段不加噪音，直接 sigmoid
        pred_edge_weight = self.concrete_sample(pred_edge_weight, 1, False)
        ...
        for edge_index, N, C in zip(edge_indices, num_edges, cum_edges):
            ...
            if adapt_select:
                ...
                elif len(idx_reserve) / N < (1 - self.ratio):
                    n_reserve = int((1 - self.ratio) * N)
                    single_mask_detach = pred_edge_weight[C:C + N].detach().cpu().numpy()
                    rank = np.argpartition(-single_mask_detach, n_reserve)
                    idx_reserve = rank[:n_reserve]
        ...
        # 重点：送入分类器前，必须 set_masks
        my_set_masks(causal_weight, self.classifier)
        _, batch_rep = self.classifier(causal_graph, get_rep=True)
        my_clear_masks(self.classifier)
